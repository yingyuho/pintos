...
int main() {
  ...
  int i, j, k;
  // Note that i, j, and k are reused a lot, sometimes not as index
  // variables. (j, at some point, becomes the number of nodes)
  int pipefds[2][2];
  ...
  for(;;) {
    ...
    if (feof(stdin)) {
      // User typed a EOF; exit
      printf("^D\n");
      strcpy(cmdbuf, "exit\n");
    }

    base = malloc(sizeof (node_t*) * 10);
    ...
    // j is the number of nodes from here on
	...
	break;
      case EACCES:
	// No execute permission, or the file or script interpreter is not a
	// file or the path can't be resolved.
	fprintf(stderr, "Permission denied\n");
	break;
      case EFAULT:
	...
	break;
      case EINVAL:
	fprintf(stderr, "ELF executable tried to name more than one"
		" interpreter\n");
	break;
      case EIO:
